...
/// Unit file template version. Embedded as `# sdtab:template_version=N` in
/// generated service files so `sdtab list` can detect legacy units that were
/// generated by an older sdtab and recommend `sdtab upgrade`.
///
/// Bump this whenever the generated unit file format changes in a way that
/// existing users should adopt (new systemd directives, layout changes, etc).
///
/// Version history:
/// - 1 (implicit): pre-SyslogIdentifier. Units without this comment are
